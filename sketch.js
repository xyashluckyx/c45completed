...
function draw() {
  ...
  obstacleGroup.setVelocityYEach((8 + 0.8*score/1));
  space.velocityY=(8 + 1*score/5);
...
if(gameState===END){
  ...
  if(gameState===START){
    ...
    text("Press Space button to go on the mission.",200,500);
  ...
  if(gameState===END){
    ...
    text("Press space button to playY again the game.",180,430);
...
function Obstacle(){
  if(frameCount%150===0){
    ...
    obstacle.scale=0.2;
    obstacle.velocityY=(5 + 0.8*score/1);;
